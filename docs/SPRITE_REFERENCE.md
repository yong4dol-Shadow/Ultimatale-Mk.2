...
* **머즐** — 얼굴 앞아래의 둥근 살색 영역. **두개골 실루엣 밖으로는 1px만 나온다**.
  더 내밀면 주둥이가 얼굴을 잡아먹고, 반대로 실루엣 안쪽으로 당기면 머즐 앞에 털이 한 겹
  둘러서 부리처럼 보이므로 실루엣과 거의 맞닿는 위치가 정답이다.
  코는 머즐 앞위쪽에 얹되 오른쪽·아래로 살색이 둘러싸야 외곽선이 아니라 코로 읽힌다.
