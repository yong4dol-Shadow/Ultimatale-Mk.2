...
* **에어 슈즈의 화염** — 주 분사는 **스케이터의 뒤쪽 한 점**에 고정하고 애니메이션
  ...
  불이 발밑에서 솟는 것처럼 보인다).
  그리고 불은 **3단계로 나눠 뽑는다** — 불 없음 / 뒤꿈치만 / 전체. 게임 쪽에서 가속
  진행도에 맞춰 골라 쓴다. 발마다 큰 불을 달면 발을 쫓아다니는 두 개의 불덩이로 읽히고,
...
* **오버월드(작은) 스프라이트** — 두상이 14px 남짓이라 전투용 머즐을 그대로 줄이면
  ...
  **검은 코뿐**이다(코를 키우는 것이 아니라 앞으로 내미는 것).
  ...
  코는 **2픽셀**이다. 이 크기에서 실루엣을 1픽셀 깨는 데는 그거면 충분하고, 소닉 계열의
  코가 하는 일이 딱 그것뿐이다. 11픽셀 두상에 5픽셀짜리 코를 달면 그건 부리다.
